...
fn slot<'a>(cfg: &'a mut Config, name: &str) -> Option<Slot<'a>> {
    ...
    Some(match name {
        ...
        "hostname" => Text(&mut cfg.mail.hostname),
        "sig_on_top" => FlagOpt(&mut cfg.mail.sig_on_top),
        "user_agent" => FlagOpt(&mut cfg.mail.user_agent),
        "pipe_sep" => Text(&mut cfg.mail.pipe_sep),
        "pipe_decode" => FlagOpt(&mut cfg.mail.pipe_decode),
        "print_decode" => FlagOpt(&mut cfg.mail.print_decode),
        ...
        "crypt_replysign" => Flag(&mut cfg.pgp.reply_sign),
        "crypt_replyencrypt" => Flag(&mut cfg.pgp.reply_encrypt),
        "crypt_replysignencrypted" => Flag(&mut cfg.pgp.reply_sign_encrypted),
        "fast_reply" => Flag(&mut cfg.mail.fast_reply),
        "askcc" => Flag(&mut cfg.mail.ask_cc),
        "confirmappend" => Flag(&mut cfg.mail.confirmappend),
        ...
        "sig_dashes" => FlagOpt(&mut cfg.mail.sig_dashes),
        "abort_nosubject" => Text(&mut cfg.mail.abort_nosubject),
        "abort_unmodified" => FlagOpt(&mut cfg.mail.abort_unmodified),
        "autoedit" => Flag(&mut cfg.mail.autoedit),
        "beep" => Flag(&mut cfg.ui.beep),
        "beep_new" => Flag(&mut cfg.ui.beep_new),
        "wait_key" => FlagOpt(&mut cfg.ui.wait_key),
        "mark_old" => FlagOpt(&mut cfg.mail.mark_old),
        "delete_untag" => FlagOpt(&mut cfg.mail.delete_untag),
        "flag_safe" => Flag(&mut cfg.mail.flag_safe),
        "maildir_trash" => Flag(&mut cfg.mail.maildir_trash),
        "mail_check_recent" => FlagOpt(&mut cfg.mail.mail_check_recent),
        "check_new" => FlagOpt(&mut cfg.mail.check_new),
        "uncollapse_new" => FlagOpt(&mut cfg.index.uncollapse_new),
        "menu_scroll" => FlagOpt(&mut cfg.ui.menu_scroll),
        ...
        "pager_stop" => Flag(&mut cfg.pager.pager_stop),
        "markers" => FlagOpt(&mut cfg.pager.markers),
        ...
        "net_timeout" => NumU64(&mut cfg.net.timeout),
